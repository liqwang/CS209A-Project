...
//    private final GitHubAPI gitHubAPI = GitHubAPI.registerAPI("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");
    ...
    private final HashMap<String,Integer> springHotmap=new HashMap<>();
                ...
                for (User user : users) {
                    ...
                    for (String key : COUNTRY_MAP.keySet()) {
                        if(location.contains(key)){
                            springHotmap.put(key,springHotmap.getOrDefault(key,0)+springCount);
                        }
                    }
                }
                logger.debug("Loading Spring hotmap success!");
            });
        }
